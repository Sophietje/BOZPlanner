...
class TestsAdminMeetingsViews(TestCase):
    ...
    def test_meetings_correct_update(self):
        ...
        resp = self.client.post(reverse('meetings:meeting-toggle', kwargs={'pk': 1}))
        self.assertEqual(resp.status_code, 200)
    ...
    def test_meetings_incorrect_update(self):
        ...
        student.save()
        resp = self.client.post(reverse('meetings:meeting-toggle', kwargs={'pk': 1}), {'secretary': student})
        self.assertEqual(resp.status_code, 200)
        meeting_1 = Meeting.objects.get(pk=1)
        self.assertEqual(meeting_1.secretary, admin)

        # Post NO data
        resp = self.client.post(reverse('meetings:meeting-update', kwargs={'pk': 1}))
        self.assertEqual(resp.status_code, 200)
        # TODO Check whether errors have occured

        # Send junk post data
        resp = self.client.post(reverse('meetings:meeting-update', kwargs={'pk': 1}), {'foo': 'bar'})
        self.assertEqual(resp.status_code, 200)

        # Send non-existent Organization pk
        resp = self.client.post(reverse('meetings:meeting-update', kwargs={'pk': 1}), {'organization': 5})
        self.assertEqual(resp.status_code, 200)
