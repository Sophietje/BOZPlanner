...
class TestUtils():
    ...
    def setupMeeting(self):
        ...
        m = Meeting.objects.create(organization=p, begin_time=datetime(2016, 11, 12, 14, 15, 0), end_time=datetime(2016, 11, 12, 15, 15, 0), place='Educafé')
        m.save()

        # Add meeting from suborganization of 'Test'
        q = Organization.objects.create(name='Subtest', parent_organization=o)
        m = Meeting.objects.create(organization=q, begin_time=datetime(2016, 11, 12, 11, 12, 0), end_time=datetime(2016, 11, 12, 12, 0, 0), place='CR 2D')
        m.save()