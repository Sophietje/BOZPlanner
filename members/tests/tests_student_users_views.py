...
class TestStudentUsersViews(TestCase, TestUserMixin):
    ...
    def test_users_update(self):
        ...
        resp = self.client.post(reverse('members:person_update', kwargs={'pk': self.user.pk}), {'first_name': 'Tester'})
        self.assertEqual(resp.status_code, 403)
        self.assertEqual(user_1, Person.objects.get(pk=self.user.pk))

        # Post junk data, user should NOT be updated
        resp = self.client.post(reverse('members:person_update', kwargs={'pk': self.user.pk}), {'test': 'onzin'})
        self.assertEqual(resp.status_code, 403)
        self.assertEqual(user_1, Person.objects.get(pk=self.user.pk))
    ...
    def test_users_delete(self):
        ...
        resp = self.client.post(reverse('members:person_delete', kwargs={'pk': self.user.pk}))
        self.assertEqual(resp.status_code, 403)
